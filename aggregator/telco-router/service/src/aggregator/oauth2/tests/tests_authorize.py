...
class AuthorizationCodeTestCase(BasicTestCase):
    ...
    def do_authorize(cls, params):
        client = Client()
        return client.get('/' + settings.AGGREGATOR_PATH_PREFIX + 'oauth2/authorize', params)

    @classmethod
    def do_callback(cls, params):
        client = Client()
        return client.get('/' + settings.AGGREGATOR_PATH_PREFIX + 'oauth2/authorize/callback', params)
...
class AuthorizationCodeOKTestCase(AuthorizationCodeTestCase):
    ...
    def test_code_ok(self, m):
        ...
        del params['state']
        params['redirect_uri'] = f"{settings.AGGREGATOR_HOST}{settings.AGGREGATOR_PATH_PREFIX}/oauth2/authorize/callback"
...
class AuthorizationCodeErrorTestCase(AuthorizationCodeTestCase):
    ...
    def test_invalid_method(self):
        client = Client()
        response = client.post('/' + settings.AGGREGATOR_PATH_PREFIX + 'oauth2/authorize')
...
class AuthorizationCodeParametersTestCase(AuthorizationCodeTestCase):
    ...
    def test_wrong_query_params(self):
        client = Client()
        response = client.get('/' + settings.AGGREGATOR_PATH_PREFIX + f'oauth2/authorize?=&&?')
    ...
    def test_duplicated_parameters(self):
        ...
        for param in ['request', 'client_id', 'response_type', 'redirect_uri', 'scope', 'state', 'nonce',
            ...
            response = client.get('/' + settings.AGGREGATOR_PATH_PREFIX + f'oauth2/authorize?{param}=foo&{param}=bar')
...
class AuthorizationCodeOperatorErrorTestCase(AuthorizationCodeTestCase):
    ...
    def test_no_params(self):
        client = Client()
        response = client.get('/' + settings.AGGREGATOR_PATH_PREFIX + 'oauth2/authorize/callback')
        self.assertErrorPage(response, 400, 'invalid_request', 'Missing state parameter.')

    def test_wrong_state(self):
        client = Client()
        response = client.get('/' + settings.AGGREGATOR_PATH_PREFIX + 'oauth2/authorize/callback', {'state': 'foo'})
