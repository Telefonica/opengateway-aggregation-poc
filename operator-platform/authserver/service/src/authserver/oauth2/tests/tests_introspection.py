...
class IntrospectTokenTestCase(BasicTestCase):
    ...
    def do_instrospection(cls, payload, headers, **kwargs):
        client = Client()
        return client.post('/' + settings.AUTHSERVER_PATH_PREFIX + 'oauth2/introspect', data=urlencode(payload), content_type='application/x-www-form-urlencoded', **headers)
...
class IntrospectTokenOKTestCase(IntrospectTokenTestCase):
    ...
    def test_json_content_type(self):
        client = Client()
        response = client.post('/' + settings.AUTHSERVER_PATH_PREFIX + 'oauth2/introspect',
...
class IntrospectTokenErrorTestCase(IntrospectTokenTestCase):
    ...
    def test_wrong_method(self):
        client = Client()
        response = client.get('/' + settings.AUTHSERVER_PATH_PREFIX + 'oauth2/introspect', self.get_token_request_parameters(), self.get_default_headers())
