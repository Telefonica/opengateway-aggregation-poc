...
class RevokeTokenTestCase(BasicTestCase):
    ...
    def do_revocation(cls, payload, headers, **kwargs):
        client = Client()
        return client.post('/' + settings.AUTHSERVER_PATH_PREFIX + 'oauth2/revoke', data=urlencode(payload), content_type='application/x-www-form-urlencoded', **headers)
...
class RevokeTokenOKTestCase(RevokeTokenTestCase):
    ...
    def test_json_content_type(self):
        ...
        self.assertIsNotNone(token)
        client = Client()
        response = client.post('/' + settings.AUTHSERVER_PATH_PREFIX + 'oauth2/revoke',
...
class RevokeTokenErrorTestCase(RevokeTokenTestCase):
    ...
    def test_wrong_method(self):
        client = Client()
        response = client.get('/' + settings.AUTHSERVER_PATH_PREFIX + 'oauth2/revoke', self.get_token_request_parameters(), self.get_default_headers())
