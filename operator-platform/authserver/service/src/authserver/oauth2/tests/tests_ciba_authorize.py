...
class CibaTestCase(BasicTestCase):
    ...
    def do_authorize(cls, payload, headers, **kwargs):
        client = Client()
        return client.post('/' + settings.AUTHSERVER_PATH_PREFIX + 'oauth2/bc-authorize', data=urlencode(payload), content_type='application/x-www-form-urlencoded', **headers)
...
class CibaErrorTestCase(CibaTestCase):
    ...
    def test_duplicated_parameters(self):
        ...
        for param in ['scope', 'login_hint_token']:
            response = client.post('/' + settings.AUTHSERVER_PATH_PREFIX + 'oauth2/bc-authorize',
