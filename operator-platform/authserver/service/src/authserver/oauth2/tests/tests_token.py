...
class TokenTestCase(BasicTestCase):
    ...
    def do_token(cls, payload, headers, **kwargs):
        client = Client()
        return client.post('/' + settings.AUTHSERVER_PATH_PREFIX + 'oauth2/token', data=urlencode(payload), content_type='application/x-www-form-urlencoded', **headers)
...
class TokenErrorTestCase(TokenTestCase):
    ...
    def test_wrong_method(self):
        client = Client()
        response = client.get('/' + settings.AUTHSERVER_PATH_PREFIX + 'oauth2/token')
